...
# Order: highest throughput → best reasoning (fallback chain).
# 2.5-flash-lite: 15 RPM / 1000 RPD — workhorse for classification
# 2.5-flash:      10 RPM / 250 RPD  — fallback if lite exhausted
# 2.5-pro:         5 RPM / 100 RPD  — highest quality, last resort
_DEFAULT_MODELS = "gemini-2.5-flash-lite,gemini-2.5-flash,gemini-2.5-pro"
MODEL_IDS = [
    m.strip()
    for m in os.environ.get("GEMINI_MODELS", _DEFAULT_MODELS).split(",")
    if m.strip()
]
