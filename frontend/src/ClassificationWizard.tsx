...
const FEATURE_GROUPS = {
  emotional: {
    label: "Емоційні",
    description: "Емоційний тон та невербальна емоційність тексту",
    features: [
      { key: "sentiment_score", label: "Тональність тексту" },
      { key: "negative_ratio", label: "Частка негативної лексики" },
      { key: "emotion_intensity", label: "Інтенсивність емоцій" },
      { key: "emoji_count", label: "Кількість емодзі" },
      { key: "exclamation_count", label: "Знаки оклику" },
    ],
  },
  stylistic: {
    label: "Стилістичні",
    ...
    features: [
      { key: "caps_ratio", label: "Частка ВЕЛИКИХ ЛІТЕР" },
      { key: "ttr", label: "Лексичне різноманіття" },
      { key: "repetition_score", label: "Повторюваність фраз" },
      { key: "avg_word_length", label: "Середня довжина слова" },
    ],
  },
  rhetorical: {
    label: "Риторичні",
    ...
    features: [
      { key: "clickbait_score", label: "Клікбейт та маніпуляції" },
      { key: "authority_refs", label: "Анонімні посилання" },
      { key: "pronoun_ratio", label: "Займенники ми/вони" },
      { key: "question_count", label: "Риторичні питання" },
    ],
  },
  social: {
    label: "Соціальні",
    ...
    features: [
      { key: "upvote_ratio", label: "Upvote ratio" },
      { key: "score_normalized", label: "Нормалізований score" },
      { key: "num_comments_norm", label: "Кількість коментарів" },
      { key: "domain_credibility", label: "Надійність домену" },
      { key: "account_age_norm", label: "Вік акаунту" },
      { key: "has_url", label: "Наявність URL" },
    ],
  },
};
...
const DEFAULT_PARAMS: any = {
  ...
  xlm_r: {
    ...
    additional_groups: ["emotional", "stylistic", "rhetorical"],
    feature_mask: buildDefaultMask(true),
  },
  llm: {
    mode: "single",
...
const S: any = {
  ...
  modeIcon: { fontSize: 36, marginBottom: 12 },
  modeTitle: { fontWeight: 700, fontSize: 17, marginBottom: 8 },
  modeDesc: { color: "#666", fontSize: 13, lineHeight: 1.5 },
  stepBar: {
    display: "flex",
    alignItems: "center",
    marginBottom: 28,
...
export default function ClassificationWizard({
  ...
  const buildRequest = () => {
    ...
    const models = modelTypes.map((type: string) => {
      ...
      if (hasAdditional) {
        model.additional_features = {
          groups: groups,
          mask: p.feature_mask || buildDefaultMask(false),
        };
      } else {
    ...
    const req: any = { mode, models, preprocessing: nbParams.preprocessing };
    if (mode === "ensemble") {
  ...
  const renderModeSelect = () => (
    <div>
      <p style={{ fontWeight: 600, marginBottom: 20, fontSize: 16 }}>
            ...
            <div>
              ...
              >
                ...
                <div style={{ gridColumn: "1 / -1", marginTop: 6 }}>
                  <span style={{ fontSize: 13, fontWeight: 500 }}>Нормалізація слів:</span>
                ...
                {[
                  {
                    id: "concat",
                    name: "Concat",
                    desc: "[CLS 768] + [features N] → Linear",
                  },
                  {
                    id: "multiview",
                    name: "Multi-view",
                    desc: "Окремий MLP на кожну групу → voting",
                  },
                ].map((m: any) => (
                  <label
                    key={m.id}
                    className={`model-option ${(p.integration_mode || "concat") === m.id ? "selected" : ""
                      }`}
                  ...
                  >
                    <input
                      ...
                      name={`xlm_mode_${type}`}
                      value={m.id}
                      checked={(p.integration_mode || "concat") === m.id}
                      onChange={() => upd("integration_mode", m.id)}
                    />
                    <div className="model-info">
                      <span className="model-name">{m.name}</span>
                      <span className="model-description">{m.desc}</span>
  ...
  const renderConfigSummary = () => {
    ...
    return (
      <div style={{ padding: 14, background: "#f0f4f8", borderRadius: 10, fontSize: 13, lineHeight: 1.8 }}>
        <p style={{ fontWeight: 700, marginBottom: 6, fontSize: 14 }}>Підсумок конфігурації</p>
        <p>
        ...
        {modelIds.map((mid: string) => {
          ...
          return (
            <div key={mid} style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #d0d8e0" }}>
              ...
              <p>
                <strong>Додаткові ознаки:</strong>{" "}
                {groups.length > 0 ? `${groups.map((g: string) => FEATURE_GROUPS[g]?.label).join(", ")} (${activeCount}/13)` : "вимкнено"}
              </p>
            </div>
          );
        })}
  ...
  const renderEnsembleStrategy = () => {
    ...
    return (
      <div>
        <p style={{ marginBottom: 16, color: "#666" }}>Оберіть стратегію об'єднання прогнозів</p>
        ...
        {ensembleStrategy === "weighted" && (
          <div style={{ marginTop: 20 }}>
            <p style={{ fontWeight: 600, marginBottom: 6 }}>Ваги моделей</p>
              ...
              return (
                <div key={mid} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontWeight: 600, fontSize: 14 }}>{MODEL_LABELS[mid]}</span>
                    <span style={{ color: "#4a90d9", fontWeight: 600, fontSize: 13 }}>
                      {norm.toFixed(2)}
                    </span>
                  </div>
                  <input
...
function AdditionalFeaturesSection({ type, groups, mask, toggleGroup, toggleFeature }: any) {
  ...
  return (
    <div>
      <p style={{ fontWeight: 600, fontSize: 14, marginBottom: 10 }}>Додаткові ознаки:</p>
      <p style={{ color: "#666", fontSize: 12, marginBottom: 12 }}>
        Семантичні (текст) завжди увімкнені. Оберіть додаткові групи ознак:
      </p>
      {Object.entries(FEATURE_GROUPS).map(([groupKey, groupDef]: any) => {
        const isActive = groups.includes(groupKey);
        const activeInGroup = isActive
        ...
        return (
          <div key={groupKey} style={{ marginBottom: 10 }}>
            <div style={S.groupToggle(isActive)} onClick={() => toggleGroup(type, groupKey)}>
              ...
              <div style={{ flex: 1 }}>
                <span style={{ fontWeight: 600, fontSize: 14 }}>{groupDef.label}</span>
                <span style={{ color: "#888", fontSize: 12, marginLeft: 8 }}>
                  {groupDef.description}
                </span>
              </div>
              {isActive && (
                <span style={{ color: "#4a90d9", fontSize: 12, fontWeight: 600 }}>
